...
class TimeSurface:
    def __init__(self, all_events, width, height, decay_const, time_step):
        self.w = width
        self.h = height
    ...
    def new_surf(self, time=0):
        ...
        for i in range(int(time/self.time_step)):
            self.update_surf_proportional()
        return self.surf
    
    def update_surf_proportional(self):
        ...
        adj_tau = self.decay_const / len(events_to_add)
